...
public class PedidoController implements Initializable {
    ...
    @FXML private Button GuardarEdit; // botón "Guardar" del FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // === OCULTAR CEROS EN LA FILA VACÍA ===
        ...
        // 👉 aplicar formato mexicano a los DatePicker
        configurarFormatoFecha(fechaEntrega);
        configurarFormatoFecha(fechaSolicitud);
    }
    ...
    private void abrirSelector(ItemFila fila) {
        try {
            ...
            ctrl.setOnProductoSeleccionado((Producto producto) -> {
    ...
    private void guardarPedido() {
        ...
        nuevo.setCliente(txtCliente.getText());
        nuevo.setTelefonoCliente(txtTelefono.getText());
        // Se guardan en formato ISO (yyyy-MM-dd), que es lo ideal para la BD
        nuevo.setFechaEntrega(fechaEntrega.getValue().toString());
        nuevo.setFechaSolicitud(fechaSolicitud.getValue().toString());

        PedidosDao pDao = new PedidosDao();
        int idPedido = pDao.insertarPedido(nuevo);

        if (idPedido == -1) {
            mostrar("Error al guardar pedido.");
            return;
        }
        ...
        for (ItemFila f : filas) {
            if (f.getId() == 0) continue; // saltar fila vacía

            DetallePedido d = new DetallePedido(
                    f.getProducto().getId(),                 // id_producto
                    f.getNombre(),                           // nombre_producto
                    f.getCantidad(),                         // cantidad
                    f.getPrecioTotal().doubleValue()         // precio (total línea)
            );
            dDao.insertarDetalle(idPedido, d);
        }

        mostrar("Pedido guardado correctamente.");
        cerrarVentana();
    ...
    // ---------- Formato mexicano para DatePicker ----------
    private void configurarFormatoFecha(DatePicker picker) {
